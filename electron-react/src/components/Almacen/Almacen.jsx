...
export const Almacen = () => {
  ...
  useEffect(() => {
    setIsLoading(true);
    axios
      .get(`${process.env.REACT_APP_API_BASE_URL}/insumos`)
      .then((datos) => {
  ...
  const handleInsumosUtilizados = async (id_insumo) => {
    ...
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_API_BASE_URL}/getInsumosUtilizadosAlmacen`,
        { id_insumo }
      );
  ...
  const filterInsumos = (insumos) => {
    const filtered = insumos.filter((insumo) => {
      ...
      } else if (filters.estado === "1") {
        // Si se selecciona "Disponible"
        return (
          insumo.cantidad_insumo - (insumo.insumos_en_uso || 0) > 0 &&
          (filters.nombre === "" || insumo.nombre_insumo === filters.nombre) &&
          insumo.tipo === "insumo"
        );
      } else if (filters.estado === "0") {
        // Si se selecciona "En uso"
        return (
  ...
  const handleSubmitDevolucionModal = async (event) => {
    ...
    try {
      ...
      const totalCantidad =
        parseInt(cantidadDevolver) + parseInt(cantidadConsumida);
      if (totalCantidad > maxCantidadDevolver) {
        setIsLoading(false);
        toast.error(
          "La suma de la cantidad a devolver y la cantidad a salir supera la cantidad máxima permitida"
        );
        return;
      }
      ...
      const devolverInsumoPromise = axios.post(
        `${process.env.REACT_APP_API_BASE_URL}/DevolverInsumo/${selectedInsumoId}`,
        {
      ...
      const salidaInsumoPromise = axios.post(
        `${process.env.REACT_APP_API_BASE_URL}/SalidaInsumoEnUso`,
        {
          id_insumo: selectedInsumoId,
      ...
      <div className="containerM">
              ...
              {paginatedInsumos.map((insumo) => {
                return (
                  <TableRow key={insumo.id_insumos}>
                    ...
                    <TableCell className="text-lg">
                      {format(
                        new Date(insumo.fecha_llegada_insumo),
                        "dd/MM/yyyy"
                      )}
                    </TableCell>
                    <TableCell className="text-lg">
                    ...
                    <TableCell
                      className="text-lg flex items-center gap-3 cursor-pointer mt-2"
                      onClick={() => handleInsumosUtilizados(insumo.id_insumos)}
                    >
                      {insumo.insumos_en_uso || 0}
                    </TableCell>
                    <TableCell className="text-lg">
                      {insumo.cantidad_insumo - (insumo.insumos_en_uso || 0)}
                    </TableCell>
                    <TableCell>
                      <Chip
                        className="capitalize text-lg p-3 rounded-lg h-10"
                        color={
                          insumo.insumos_en_uso === insumo.cantidad_insumo
                            ? "danger"
                            : "success"
                        }
                        size="sm"
                        ...
                        <Dropdown>
                          <DropdownTrigger>
                            <Button isIconOnly size="sm" variant="light">
                              <svg
                                className="w-6 h-6 text-gray-800 hover:text-lime-500 dark:text-white"
                                aria-hidden="true"
                                fill="none"
                                focusable="false"
                                role="presentation"
                          ...
                          <DropdownMenu>
                            <DropdownItem
                              onClick={() =>
                                handleDevolverInsumo(insumo.id_insumos)
                              }
                            >
                              Devolver
                            </DropdownItem>
                            <DropdownItem
                              onClick={() =>
                                handleInsumosUtilizados(insumo.id_insumos)
                              }
                            ...
                            <DropdownItem
                              onClick={() => handleVerNotas(insumo.id_insumos)}
                            >
                              Notas
                            </DropdownItem>
                          </DropdownMenu>
        ...
        {modalDevolucionVisible && (
          <div className="modal-insumos">
            <form
              onSubmit={handleSubmitDevolucionModal}
              className="form-modal-insumos"
            >
              ...
              <div className="inp-registro-DI">
                <label>
                  Nombre
                  <Input value={selectedInsumoNombre} readonly />
                </label>
                <label className="flex gap-2 items-center">
                  Cantidad a devolver{" "}
                  <p className="text-lg">(máximo {maxCantidadDevolver})</p>
                </label>
                <Input
                  type="number"
              ...
              <div className="btn-terminar-registro">
                <a
                  onClick={() => setModalDevolucionVisible(false)}
          ...
          <div className="modal-insumos">
            <div className="form-modal-insumos">
              ...
              <div className="btn-terminar-registro">
                <a
                  className="boton-cancelar-registro"
                  onClick={() => setModalVisibleInsumoU(false)}
                >
                  <h3>⮜ ‎ Atrás</h3>
                </a>
              </div>
